#!/bin/python3
#Develop by python3.10.4
import sys
import pandas

'''
杀软字典数据来源: https://github.com/gh0stkey/avList/blob/master/avlist.js
在实际使用中发现这个字典存在一些错误, 所以简单做了一些校对和补充
'''
banner = '''
Usage: python avd.py <tasklist.txt>
'''
if len(sys.argv) < 2:
    print(banner)
    exit()
tasklist = sys.argv[1]
#以多个空格为分隔读取tasklist
date = pandas.read_csv(tasklist, sep='\s{2,}', encoding='utf-16', engine='python')
#取出是进程名的列并转化为列表，转换后的列表中的元素也是列表
process_list = date.iloc[:,0:1].values.tolist()
#去除重复的进程名
simplified_list = []
for item in process_list:
    if item not in simplified_list:
...
av_dict = {
	...
	"cpf.exe": "Comodo Anti-virus",
    "cis.exe": "Comodo Anti-virus",
    "vkise.exe": "Comodo Anti-virus网页保护程序",
	...
	"naprdmgr.exe": "McAfee",
	"cleanup.exe": "McAfee",
	"cmdagent.exe": "COMODO Anti-virus",
	"frminst.exe": "McAfee",
	...
	"beagle.exe": "Avast Anti-virus",
    "AvEmUpdate.exe": "Avast Anti-virus",
	"blackd.exe": "BlackICE",
	...
	"VBA32ECM.exe": "VBA32 Anti-virus",
	"VBA32IFS.exe": "VBA32 Anti-virus",
	"VBA32PP3.exe": "VBA32 Anti-virus",
...
#把key转为小写
av_dict_pure = {}
for i, j in av_dict.items():
    av_dict_pure[i.lower()] = j
del av_dict

#取出字典av中的每个key并与列表tasklist中的每个元素做对比
for key, value in av_dict_pure.items():
    for i in range(len(pure_list)):
        if pure_list[i] == key:
            print(f"\t进程: {key}\t服务: {value}")
