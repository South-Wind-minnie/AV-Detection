...
tasklist = sys.argv[1]
#识别文件编码方式
with open(tasklist,'rb') as f:
	cur_encoding = chardet.detect(f.read())['encoding']
#以多个空格为分隔读取tasklist
date = pandas.read_csv('tasklist.txt', sep='\s{2,}', encoding='utf-16', engine='python')
#取出是进程名的列并转化为列表，转换后的列表中的元素也是列表
process_list = date.iloc[:,0:1].values.tolist()
#取出列表中的元素并转为小写
temp_list = [i.lower() for item in process_list for i in item]
del process_list
#转换为集合来删除重复项
pure_list = set(temp_list)
...
for key, value in av_dict_pure.items():
    for i in pure_list:
        if i == key:
            print(f"\tprocess: {key}\tservice: {value}")
